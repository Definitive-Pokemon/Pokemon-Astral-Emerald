...
#define P_POKERUS_STRAIN_DISTRIBUTION    GEN_6 // Pokérus has 16 different strains and their probability distribution change depending on generation, GEN_3 will use the Ruby/Sapphire version and GEN_4 will use the version used in Emerald and Gen 4
#define P_POKERUS_SPREAD_ADJACENCY       GEN_6 // In Gen 2, Pokérus spread to one adjacent Pokémon but it spreads to both adjacent Pokémon in gen 3+
#define P_POKERUS_SPREAD_DAYS_LEFT       GEN_6 // In Gen 2, a freshly spreaded Pokémon will get its full infection duration based on strain. In gen 3+, the Pokérus duration will copy the duration from the Pokémon it was spreaded from
#define P_POKERUS_INFECT_AGAIN           GEN_6 // If Gen 2 only, your party can get infected even when it is already infected with Pokérus (doesn't affect spreading)
