...
#define B_MULTI_HIT_CHANCE          GEN_6 // In Gen5+, multi-hit moves have different %. See SetRandomMultiHitCounter for values.
...
#define B_LEVEL_UP_NOTIFICATION     GEN_6 // In Gen9+, if the Pokémon gets enough experience to level up multiple times, the message is only displayed once.

// Stat settings
#define B_BADGE_BOOST               GEN_6 // In Gen4+, Gym Badges no longer boost a Pokémon's stats. (Gen2 does not include the additional boost to the type matching the gym the badge is from)
...
#define B_PSYWAVE_DMG               GEN_6 // Psywave's damage formula. See DoFixedDamageMoveCalc for details.
...
#define B_MULTIPLE_TARGETS_DMG      GEN_6 // In Gen4+, damage dealt by moves that hit multiple targets at once is reduced to 75%. In Gen3, it was 50%, unless the move hit the entire field, in which case there was no reduction.
...
#define B_SLEEP_TURNS               GEN_6 // In Gen5+, sleep lasts for 2-4 turns instead of 2-5 turns.
...
#define B_REDIRECT_ABILITY_ALLIES   GEN_6 // In Gen4+, Lightning Rod/Storm Drain redirect ally's moves as well.
...
#define B_PICKUP_WILD               GEN_6 // In Gen9+, Pickup allows its user to pickup its own used item at the end of the turn in wild battles.
#define B_MAGIC_GUARD               GEN_6 // In Gen4 only, Magic Guard ignores immobilization caused by paralysis
#define B_BATTLE_BOND               GEN_6 // In Gen9+, Battle Bond increases Atk, SpAtk and Speed by one stage, once per battle
...
#define B_DEFIANT_STICKY_WEB        GEN_6 // In Gen9+, Defiant activates on Sticky Web regardless of who set it up. In Gen8, Defiant does not activate on Sticky Web set up by an ally after Court Change swaps its side.
#define B_POWDER_OVERCOAT           GEN_6 // In Gen6+, Overcoat blocks powder and spore moves from affecting the user.
#define B_INFILTRATOR_SUBSTITUTE    GEN_6 // In Gen6+, Infiltrator bypasses Substitute when using a move, excluding Transform and Sky Drop.
...
// Item settings
#define B_CONFUSE_BERRIES_HEAL      GEN_6 // In Gens3-6, Figy and similar berries restore 1/8th of HP and trigger at half HP. In Gen7 they restore half HP, triggering at 25% HP. In Gen8 they heal 1/3rd of HP.
...
#define B_STEAL_WILD_ITEMS          GEN_6 // In Gen9+, Thief and Covet steal a wild pokemon's item and send it to the bag. In Gens2-8, the stolen item would be held by the Thief/Covet user.
...
#define B_DIVE_BALL_MODIFIER        GEN_6 // In Gen4+, Dive Ball's effectiveness increases by x3.5 when Surfing or Fishing.
...
#define B_FRIEND_BALL_MODIFIER      GEN_6 // In Gen8+, Friend Ball's friendship boost was reduced from 200 to 150.
#define B_SERENE_GRACE_BOOST        GEN_6 // In Gen5+, Serene Grace boosts the added flinch chance of King's Rock and Razor Fang.
#define B_IRON_BALL                 GEN_6 // In Gen5+, Flying-type Pokemon holding Iron Ball take x1 damage from Ground-type moves regardless of their other types, except during Inverse Battles or if the Pokemon is grounded by any other effect.
...
#define B_SANDSTORM_SOLAR_BEAM          GEN_6 // In Gen3+, Sandstorm decreases the power of Solar Beam, when it didn't before.
...
#define B_NATURE_POWER_MOVES        GEN_6 // Nature Power calls different moves depending on terrain and generation. See gBattleEnvironmentInfo.
...
#define B_CRITICAL_CAPTURE_IF_OWNED     GEN_6 // In Gen9, a capture appear critical if the pokemon you are trying to catch already has a dex entry (has already been caught)
